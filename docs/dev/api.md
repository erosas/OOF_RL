...
Fetches ranked MMR data for the player. Results are cached for 5 seconds by the internal MMR provider cache (burst protection only — MMR changes every match, so any real revisit re-fetches); the TTL is not user-configurable. Provider lookups are bounded by a 15-second request timeout and return `504` when the lookup times out.
